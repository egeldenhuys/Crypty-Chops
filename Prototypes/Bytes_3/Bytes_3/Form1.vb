...
Public Class Form1
    ...
    Private Function GetBytes(path As String) As Byte()
        ...
        'Dim hglobal As IntPtr = Marshal.AllocHGlobal(1073741824)

        ' Create a new file FileStream for the specified path
        Dim fsSource As New FileStream(path, FileMode.Open, FileAccess.Read)

        Dim buffer As Integer = CInt(txtBuffer.Text) '56 MB

        'Dim buffer As Integer = 1073741824 ' 1GB
        Dim bytes(0) As Byte

        Try
            ' Create an array of bytes for reading into. The length is the same as the source file.
            ReDim bytes(buffer) ' Array for storing the bytes

            cWriteLine("File Size: " & fsSource.Length / 1000 & " KB")

            Dim numBytesToRead As Long = fsSource.Length ' How many bytes we have to read
            Dim numBytesRead As Long = 0 ' How many bytes we have already read.
            ...
            While (numBytesToRead > 0) ' Keep reading if we still have bytes left to read.
                ...
                If bufferCount <> 0 Then
                    'cWriteLine("Clearing Buffer. (" & bufferCount & ") - " & numBytesRead / 1048576 & " MB")
                    Me.Refresh()
                End If
                ...
                ' Read 1MB at a time and discard.

                Dim n As Integer = fsSource.Read(bytes, 0, buffer) ' Reads from numBytesRead to numBytesToRead and stores it in bytes().
